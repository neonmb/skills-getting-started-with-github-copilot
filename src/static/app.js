document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    try {
      ...
      // Clear activity select dropdown and reset to default option
              ...
              details.participants && details.participants.length > 0
                ? `<h5>Participants (${details.participants.length})</h5>
                   <ul class="participants-list">
                     ${details.participants.map((p) => `<li class="participant"><span class="participant-name">${p}</span><button class="delete-btn" data-activity="${name}" data-email="${p}" title="Remove participant">&times;</button></li>`).join("")}
                   </ul>`
